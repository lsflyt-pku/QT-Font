...
class Points:
  ...
  Args:
    points (torch.Tensor): The coordinates of the points with a shape of
        :obj:`(N, 2)`, where :obj:`N` is the number of points.
  ...
  def orient_normal(self, axis: str = 'x'):
    ...
    Args:
      axis (int): The coordinate axes, choose from :obj:`x` and :obj:`y`. (default: :obj:`x`)
    '''

    if self.normals is None:
      return

    axis_map = {'x': 0, 'y': 1, 'xyz': 2}
    idx = axis_map[axis]
    if idx < 2:
  ...
  def translate(self, dis: torch.Tensor):
    ...
    Args:
      dis (torch.Tensor): The displacement with shape :obj:`(2,)`.
    '''
  ...
  def flip(self, axis: str):
    ...
    Args:
      axis (str): The flipping axis, choosen from :obj:`x` and :obj:`y`.
    '''
  ...
  def __getitem__(self, mask: torch.Tensor):
    ...
    dummy_pts = torch.zeros(1, 2, device=self.device)
  ...
  def to(self, device: Union[torch.device, str], non_blocking: bool = False):
    ...
    points = Points(torch.zeros(1, 2, device=device))
