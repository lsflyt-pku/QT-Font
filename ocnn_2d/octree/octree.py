...
from ocnn_2d.utils import meshgrid, scatter_add, cumsum, trunc_div
...
class Octree:
  ...
  def reset(self):
    ...
    center_grid = self.rng_grid(2, 3)    # (4, 2)
    displacement = self.rng_grid(-1, 1)  # (9, 2)
    
    
    # neigh_grid = center_grid.unsqueeze(1) + displacement  # (8, 27, 3)
    neigh_grid = center_grid.unsqueeze(1) + displacement  # (4, 9, 2) 4 表示周围有4个邻域点 (中心点是0,0), 9表示网格点坐标（3x3), 2是坐标维度(x,y)
    ...
    self.lut_parent = torch.sum(
        parent_grid * torch.tensor([3, 1], device=device), dim=2) # 位移多少位，
    self.lut_child = torch.sum(
        child_grid * torch.tensor([2, 1], device=device), dim=2)

    # lookup tables for different kernel sizes
    # 默认是333直接返回neighbor, 不用改这部分
    # self.lut_kernel = {
  ...
  def build_octree(self, point_cloud: Points):
    ...
    for d in range(self.depth, self.full_depth, -1):
      ...
      # augmented key
      key = (pkey.unsqueeze(-1) << 2) + torch.arange(4, device=self.device)
      self.keys[d] = key.view(-1)
      self.nnum[d] = key.numel()
      self.nnum_nempty[d] = node_key.numel()

      # children
      addr = (pidx << 2) | (node_key % 4)
      children = -torch.ones(
    ...
    nempty_idx = node_key if self.batch_size == 1 else  \
        ((node_key >> 48) << (2 * d)) | (node_key & ((1 << 48) - 1))
  ...
  def octree_grow(self, depth: int, update_neigh: bool = True):
    ...
    nnum = self.nnum_nempty[depth-1] * 4
    ...
    key = (key & ((1 << 48) - 1)) << 2
    key = key | batch_id
    key = key.unsqueeze(1) + torch.arange(4, device=key.device)
    self.keys[depth] = key.view(-1)
  ...
  def construct_neigh(self, depth: int):
    r''' Constructs the :obj:`2x2x2` neighbors for each octree node.
    ...
    if depth <= self.full_depth:
      ...
      x, y, _ = key2xyz(key, depth)
      xyz = torch.stack([x, y], dim=-1)  # (N,  2)
      grid = self.rng_grid(min=-1, max=1)   # (9, 2)
      
      xyz = xyz.unsqueeze(1) + grid         # (N, 9, 2)
      xyz = xyz.view(-1, 2)                 # (N*9, 2)
      neigh = xyz2key(xyz[:, 0], xyz[:, 1], depth=depth)

      bs = torch.arange(self.batch_size, dtype=torch.int32, device=self.device)
      neigh = neigh + bs.unsqueeze(1) * nnum  # (N*9,) + (B, 1) -> (B, N*9)

      bound = 1 << depth
      invalid = torch.logical_or((xyz < 0).any(1), (xyz >= bound).any(1))
      neigh[:, invalid] = -1
      self.neighs[depth] = neigh.view(-1, 9)  # (B*N, 9)

    else:
      child_p = self.children[depth-1]
      nempty = child_p >= 0
      neigh_p = self.neighs[depth-1][nempty]   # (N, 9)
      
      neigh_p = neigh_p[:, self.lut_parent]    # (N, 4, 9)
      child_p = child_p[neigh_p]               # (N, 4, 9)
      invalid = torch.logical_or(child_p < 0, neigh_p < 0)   # (N, 4, 9)
      neigh = child_p * 4 + self.lut_child
      neigh[invalid] = -1
      self.neighs[depth] = neigh.view(-1, 9) # 同上
  ...
  def search_xyzb(self, query: torch.Tensor, depth: int, nempty: bool = False):
    ...
    key = xyz2key(query[:, 0], query[:, 1], query[:, 2], depth)
  ...
  def get_neigh(self, depth: int, kernel: str = '333', stride: int = 1,
    ...
    if stride == 1:
      ...
      neigh = self.neighs[depth][::4].clone()
    else:
  ...
  def to_points(self, rescale: bool = True):
    ...
    if xyz is None:
      x, y, batch_id = self.xyzb(depth, nempty=True)
      xyz = torch.stack([x, y], dim=1) + 0.5
  ...
  def rng_grid(self, min, max):
    ...
    grid = meshgrid(rng, rng, indexing='ij')
    grid = torch.stack(grid, dim=-1).view(-1, 2)  # (9, 2)
    return grid
