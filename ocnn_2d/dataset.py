...
class CollateBatch:
  ...
  def __call__(self, batch: list):
    ...
    for key in batch[0].keys():
      ...
      if 'octree' in key:
        if type(outputs[key][0]) == list:
          octree_list = []
          ref_num = len(outputs[key][0])
          L = len(outputs[key])
          for i in range(ref_num):
            octree = ocnn_2d.octree.merge_octrees([outputs[key][x][i] for x in range(L)])
            octree.construct_all_neigh()
            octree_list.append(octree)
          outputs[key] = octree_list
        else:
          octree = ocnn_2d.octree.merge_octrees(outputs[key])
      ...
      if 'points' in key and self.merge_points:
        outputs[key] = ocnn_2d.octree.merge_points(outputs[key])
