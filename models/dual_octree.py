...
from ocnn_2d.octree import key2xyz, xyz2key
from ocnn_2d.utils import cumsum
...
class DualOctree:
  def __init__(self, octree):
    ...
    x,y,i = xyzi
    self.xyzi = torch.stack((x,y,i),dim=1)
    self.xyz = self.xyzi[:, :2]
    self.batch = self.xyzi[:, 2]
  ...
  def _internal_edges(self, nnum, dis=0):
    assert(nnum % 4 == 0)
    d = torch.arange(0, nnum / 4, dtype=torch.int64, device=self.device)
    d = torch.unsqueeze(d * 4 + dis, dim=1)
  ...
  def relative_dir(self, vi, vj, depth, rescale=True):
    ...
    # get 4 neighborhoods of xi via `self.ngh`
  ...
  def sparse_graph(self, depth, graph):
    ...
    row_o1 = self.child[vi].unsqueeze(1) * 4 + self.dir_table[rel_dir, :]
    row_o1 = row_o1.view(-1) + ncum_d  # NOTE: add ncum_d
    col_o1 = vj.unsqueeze(1).repeat(1, 2).view(-1)
    dir_o1 = rel_dir.unsqueeze(1).repeat(1, 2).view(-1)
    ...
    if invalid_both_vtx.any():
      ...
      row_o2 = self.child[vi].unsqueeze(1) * 4 + self.dir_table[rel_dir, :]
      row_o2 = row_o2.view(-1) + ncum_d  # NOTE: add ncum_d
      dir_o2 = rel_dir.unsqueeze(1).repeat(1, 2).view(-1)
      rel_dir_col = self.remap[rel_dir]
      col_o2 = self.child[vj].unsqueeze(1) * 4 + self.dir_table[rel_dir_col, :]
  ...
  def add_self_loops(self):
    for d in range(self.full_depth, self.depth + 1):
      ...
      dir_idx = torch.ones_like(unique_idx) * 4
  ...
  def get_input_feature(self, all_leaf_nodes=True, label='L'):
    # the initial feature of leaf nodes in the layer self.depth
    octree_feature = InputFeature(feature = label, nempty=False)
  ...
  def add_node_type(self):
    ...
    for i, d in enumerate(range(full_depth, depth + 1)):
      ...
      # print('node_type', d, self.graph[d]['node_type'].shape)
  ...
  def add_node_mask(self):
    ...
    for i, d in enumerate(range(full_depth, depth + 1)):
      ...
      # print(d, self.graph[d]['node_mask'].shape)
...
if __name__ == '__main__':
  import ocnn_2d
  octrees = ocnn_2d.octree_samples(['octree_1', 'octree_2'])
  octree = ocnn_2d.octree_batch(octrees).cuda()
