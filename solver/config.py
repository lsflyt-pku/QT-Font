...
_C.DATA.train.canny         = False       # Calculate the node feature
_C.DATA.train.ref_num = 3
_C.DATA.train.c_ref_num = 1
...
# For data scheduler
_C.DATA.train.beta_start    = 0.0
_C.DATA.train.beta_end      = 1.0
_C.DATA.train.num_train_timesteps = 1000
_C.DATA.train.beta_schedule = 'linear'
...
_C.MODEL.depth_stop         = 6           # The stop depth of auto encoder
...
_C.MODEL.resblk_num         = 3           # The resblock number
...
_C.MODEL.code_channel       = 3      # The dropout ratio

_C.MODEL.cond = 'uncond'


_C.MODEL.embed_dim          = 3           # embed_dim of quantizer
_C.MODEL.n_embed            = 8192        # n_embed of quantizer
...
def _update_config(FLAGS, args):
  ...
  # solver_flags = FLAGS.SOLVER
  # if solver_flags.run == 'evaluate':
  #   FLAGS.SOLVER.logdir = os.path.join('logs','shapenet_eval',f'{model_flags.name}-full_depth-{model_flags.full_depth}-resblk_num-{model_flags.resblk_num}-code_channel-{model_flags.code_channel}')
  # elif solver_flags.run == 'train':
  #   FLAGS.SOLVER.logdir = os.path.join('logs','shapenet',f'{model_flags.name}-full_depth-{model_flags.full_depth}-resblk_num-{model_flags.resblk_num}-code_channel-{model_flags.code_channel}')
...
def parse_args(backup=True):
  ...
  print(FLAGS.SOLVER.run)
  if backup and FLAGS.SOLVER.run == 'train':
